...
window.onload = function () {
    ...
    function CreateVueData(inputData) {
        ...
        vue = new Vue({
            ...
            methods: {
                CreatePartnersList: function(data) {
                    if (data) {
                        if (data.partnersList) this.partnersList = data.partnersList;
                        console.log(this.partnersList)
                    }
                },
                GetPartnersList: function() {
                    let partnersList = new XMLHttpRequest();
                    partnersList.open('GET', '/partners/list', true);
                    partnersList.setRequestHeader("Content-Type", "application/json");
                    partnersList.onreadystatechange = function () {
                        if (partnersList.readyState == 4)
                        if (partnersList.status === 200) {
                            vue.CreatePartnersList(JSON.parse(partnersList.responseText));
                            $('#back').css('display', 'none');
                        } else {
                            ShowMsg(errorMsg);
                            $('#back').css('display', 'none');
    ...
    function sendFormButtonClick() {
        ...
        vue.inputData.vacancy = CheckVacancy(DeleteEmptyElements(vue.inputData.vacancy));
    ...
    function CheckVacancy(array) {
        let currentArray = [];
        $.each(array, function (i, obj) {
            if (obj.name && obj.desk)
                currentArray.push(obj);
        });
        return currentArray;
    }
